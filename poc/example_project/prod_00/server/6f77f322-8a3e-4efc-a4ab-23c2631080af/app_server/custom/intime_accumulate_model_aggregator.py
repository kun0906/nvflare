...
class InTimeAccumulateWeightedAggregator(Aggregator):
    ...
    def accept(self, shareable: Shareable, fl_ctx: FLContext) -> bool:
        ...
        client_model_file = os.path.join(fl_ctx.get_prop("__app_root__"), 'models',
                                          f'{contributor_name}_weights_{current_round}.pkl')
        client_model_file = os.path.abspath(client_model_file)
        model_dir = os.path.dirname(client_model_file)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir, exist_ok=True)
        self.log_info(fl_ctx, f'clients_model_file: {client_model_file}')
        torch.save(dxo, client_model_file)
