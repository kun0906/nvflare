...
def client_i_training(X_train, y_train, l, gans, global_gan):
    # print(f'\nclient_{client_i}')
    # l = client_i
    ...
    X = X_train[label_mask]
    y = y_train[label_mask]
    ...
    # lr = 0.0002
    local_gan = Generator()
    local_gan.load_state_dict(global_gan.state_dict())
    ...
    for epoch in range(client_epochs):
        ...
        # ---- Logging ----
        if epoch % 100 == 0:
            print(f"Epoch {epoch}/{client_epochs} | D Loss: {d_loss.item():.4f} | G Loss: {g_loss.item():.4f} |  "
        ...
        if epoch % 100 == 0:
            ...
            with torch.no_grad():
                ...
                plt.suptitle(f'epoch {epoch}, class: {l}')
                plt.tight_layout()
                fig_file = f'tmp/sep/generated_{l}.png'
...
def train_gan(X_train, y_train, X_val, y_val, X_test, y_test):
    global_gan = Generator()
    global_gan = global_gan.to(device)

    SERVER_EPOCHS = 100
    for server_epoch in range(SERVER_EPOCHS):
        print(f'\nserver epoch: {server_epoch}')
        gans = {l: Generator().state_dict() for l in LABELs}
        for l in LABELs:  # we have 10 clients
            generator_params = client_i_training(X_train, y_train, l, gans, global_gan)
            gans[l] = generator_params

        #################
        # aggregate parameters

        clients_parameters = {l: m for l, m in gans.items()}
        # Initialize the aggregated state_dict for the global model
        global_state_dict = {key: torch.zeros_like(value).to(device) for key, value in
                             global_gan.state_dict().items()}
        ...
        for key in global_state_dict:
            ...
            clients_updates = [client_state_dict[key] for client_state_dict in clients_parameters.values()]
            ...
            global_state_dict[key] = aggregated_update / len(clients_updates)

        # Update the global model with the aggregated parameters
        global_gan.load_state_dict(global_state_dict)
        ...
        with torch.no_grad():
            ...
            plt.suptitle(f'server epoch {server_epoch}')
            plt.tight_layout()
            fig_file = f'tmp/sep/generated_server_{server_epoch}.png'
            dir_path = os.path.dirname(os.path.abspath(fig_file))
            os.makedirs(dir_path, exist_ok=True)
            plt.savefig(fig_file)
            plt.show()
        ...
        torch.save(global_gan, f'tmp/sep/global_gan_{server_epoch}.pth')
    return global_gan
...
def main():
    ...
    if os.path.exists(gan_file):
        os.rename(gan_file, gan_file+'_backup')
    if os.path.exists(gan_file):
