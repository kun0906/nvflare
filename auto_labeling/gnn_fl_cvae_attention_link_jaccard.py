...
def parse_arguments():
    ...
    parser.add_argument('-n', '--server_epochs', type=int, required=False, default=64,
...
def train_gnn(local_gnn, global_cvae, global_lp, global_gnn, local_data, train_info={}):
    ...
    if len(ct.keys()) < len(LABELs):
        ...
        for l in LABELs:
            if l in ct.keys():
                ...
                sizes[l] = max_size
                labeled_cnt[l] = max_size
        print(f'we need to generate samples ({sum(sizes.values())}),where ach class sizes:{sizes}')
    ...
    train_info['cosine_threshold'] = 1.0  #
