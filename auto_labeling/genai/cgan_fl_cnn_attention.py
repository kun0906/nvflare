...
def gen_local_data(client_data_file, client_id, label_rate=0.1):
    ...
    shared_X = torch.tensor(client_data['shared_data']['X'])  # global data x
    shared_y = torch.tensor(client_data['shared_data']['y'])  # global data y
...
def train_gan(local_gan, global_cgan, local_data, global_cnn, train_info={}):
...
def train_cnn(local_cnn, global_cgan, global_lp, global_cnn, local_data, train_info={}):
    """
    ...
    if not debug and len(ct.keys()) < len(LABELS):
        ...
        # generated new data
        generated_data = gen_data(global_cgan, sizes, similarity_method=train_info['edge_method'],
    ...
    for epoch in range(epochs_client):
        ...
        outputs = local_cnn(node_features)
...
def aggregate_gans(gans, locals_info, global_gan, local_data, histories_server, server_epoch):
    if server_epoch > 0:
        return

    aggregate_label_gans(gans, locals_info, global_gan)

    # if server_epoch == 0:
    #     train_mask = local_data['_data']['train_mask']
    #     sizes = {l: s for l, s in collections.Counter(local_data['all_data']['y'][train_mask].tolist()).items()}
    #     print(sizes)
    #     # generated new data
    #     generated_data = gen_data(global_gan, sizes, similarity_method='cosine',
    #                               local_data=local_data)
...
def aggregate_cnns(cnns, global_cnn, histories_server, epoch):
    print('*aggregate cnn...')
...
def plot_img(generated_imgs, l, show=True):
    if show:
...
def print_histories(histories):
    ...
    for model_type in ['global', 'local']:
        ...
        if model_type == 'global':
            title = f'{model_type}_cnn' + '$_{' + f'{num_server_epoches}' + '}$' + f':{label_rate}'
        else:
            title = f'{model_type}_cnn' + '$_{' + f'{num_server_epoches}+1' + '}$' + f':{label_rate}'
        plt.suptitle(title)
    ...
    if debug:
        ...
        for epoch in range(server_epochs):
            ...
            for c in range(NUM_CLIENTS):
                ...
                print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
                print('Load data...')
                train_info = {"gan": {}, "cnn": {}, 'client_id': c, 'server_epoch': epoch}  # might be used in server
                ...
                if os.path.exists(global_cgan_path):
                    ...
                    if epoch == 0:
                        ...
                        print('Train cgan...')
                        train_gan(local_cgan, global_cgan, local_data, global_cnn, train_info)
                ...
                train_cnn(local_cnn, global_cgan, None, global_cnn, local_data, train_info)
                clients_cnns[c] = local_cnn
            ...
            if not os.path.exists(global_cgan_path):
                aggregate_gans(clients_gans, locals_info, global_cgan, local_data, histories['server'], epoch)

            aggregate_cnns(clients_cnns, global_cnn, histories['server'], epoch)
