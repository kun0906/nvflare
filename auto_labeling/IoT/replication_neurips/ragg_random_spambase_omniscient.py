...
if ATTACK_METHOD == 'omniscient':  # case 2
    ...
    NUM_BYZANTINE_CLIENTS = int(0.33 * TOTAL_CLIENTS)  # 2 + 2f < n, so f <= int((n-3)/2)
...
def print_all(all_histories, precision=4):
    ...
    for model_type in ['global', 'local']:
        ...
        NUM_CLIENTS = len(all_histories['clients'][0])
        for idx_client in range(NUM_CLIENTS):
            ...
            for idx_server_epoch in range(SERVER_EPOCHS):
                ...
                for data_type in ['train', 'val', 'test', 'shared']:
                    ...
                    res = all_histories['clients'][idx_server_epoch][idx_client][f'{model_type}_{data_type}_accuracy']  # model type
                    mu, std = res, 0
...
# dirichlet_split(X, y, num_clients, alpha=0.5, random_state=42)
...
def gen_client_spambase_data(data_dir='data/spambase', out_dir='.'):
    ...
    Xs, Ys = dirichlet_split(X, y, num_clients=TOTAL_CLIENTS, alpha=BIG_NUMBER, random_state=42)
...
def main():
    ...
    if dataset == 'spambase':
        ...
        sub_dir = (f'data/spambase_neurips/random_noise_omniscient/h_{NUM_HONEST_CLIENTS}-b_{NUM_BYZANTINE_CLIENTS}'
