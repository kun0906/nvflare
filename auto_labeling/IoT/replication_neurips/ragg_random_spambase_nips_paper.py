...
import ragg
from ragg import robust_aggregation
from ragg import classical_algs_comparison
...
def parse_arguments():
    ...
    parser.add_argument('-s', '--server_epochs', type=int, required=False, default=50,
...
def aggregate_cnns(clients_cnns, clients_info, global_cnn, aggregation_method, histories, epoch):
    ...
    # clients_weights = torch.tensor([vs['size'] for vs in clients_info.values()])
    if aggregation_method == 'adaptive_krum':
        aggregated_update, clients_type_pred = ragg.robust_aggregation.adaptive_krum(flatten_clients_updates, clients_weights,
...
def gen_client_spambase_data(data_dir='data/spambase', out_dir='.'):
    ...
    Xs, Ys = dirichlet_split(X, y, num_clients=TOTAL_CLIENTS, alpha=10.0, random_state=42)
