...
def generate_non_iid_data(dataset, num_clients=10, rate=0.5):
    ...
    if rate == 0:
        ...
        for client_id in range(num_clients):
            ...
            label = client_id+1
            client_indices = client_indices[label]
        ...
        for client_id in range(num_clients):
            ...
            for class_id, indices in class_indices.items():
                ...
                # Calculate how many samples each client should get from the current class
                start_idx = (client_id * num_class_samples) % total_class_samples  # Wrap around if necessary
                end_idx = start_idx + num_class_samples
                ...
                other_clients = [i for i in range(num_clients) if i != client_id]
                ...
                for other_client in other_clients:
                    start_idx = (other_client * num_other_samples_per_client) % remaining_class_samples
                    end_idx = start_idx + num_other_samples_per_client
                    client_datasets[other_client].extend(remaining_indices[start_idx:end_idx])
...
rate = 0  # Adjust between 0 (IID) and 1 (Extreme Non-IID)
