...
class FNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc33(x)        # we use nn.CrossEntropyLoss(reduction='mean').
...
def evaluate_shared_test(local_cnn, local_data, global_cnn,
    ...
    for model_type, model in [('global', global_cnn), ('local', local_cnn)]:
        ...
        with torch.no_grad():
            ...
            if verbose >= 10:
                print(f"{model_type} Accuracy on shared test data: {accuracy * 100:.2f}%, {collections.Counter(y.tolist())}")
...
@timer
def evaluate_global_model(global_cnn, shared_data, DEVICE = 'cpu',
                         test_type='shared_test_data', train_info={}, verbose=0):
    """
        Evaluate how well each client's model performs on the test set.
    """
    # CFG = train_info['CFG']
    ...
    global_acc = 0
    for model_type, model in [('global', global_cnn)]:
        # After training, the model can make predictions for both labeled and unlabeled nodes
        if verbose >= 20:
            print(f'***Testing {model_type} model on {test_type}...')
        # evaluate the data
        ...
        if verbose >= 20:
            print(f"Server evaluation on {test_type} Accuracy: {accuracy * 100:.2f}%")
