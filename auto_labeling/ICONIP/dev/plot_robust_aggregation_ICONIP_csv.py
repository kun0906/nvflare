...
def plot_ax_attack(ax, case, JOBID, start=0):
    global_accs = {}
    # in_file = 'result-20250506/Large Outlier-299530.csv'
    in_file = f'result-20250506/{case}-{JOBID}.csv'
    import pandas as pd

    # Load CSV (change delimiter if needed)
    df = pd.read_csv(in_file, sep="|", header=None, names=["method", "file", "mean", 'std'])
    print(df.head())

    import ast
    # Safely evaluate the "(...)" string into tuple
    df["mean"] = df["mean"].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)
    print(df.head())
    for i in range(len(df)):
        method, text_file= df.iloc[i, 0], df.iloc[i, 1]
        global_accs[(method, text_file)] = {'shared_acc': list(df.iloc[i, 2])}
    ...
    for i in range(len(aggregation_methods)):
        ...
        #     raise NotImplementedError(METRIC)
        # ys, ys_errs = zip(*vs)
        ys = global_accs[(agg_method, txt_file)]['shared_acc']
