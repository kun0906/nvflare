...
1. HPC Instructions:
    ...
    $PYTHONPATH=.:nsf python3 nsf/fl_cnn_robust_aggregation_random_noise_model_newscategory.py
...
@timer
def gen_client_data(data_dir='data/NewsCategory', out_dir='.', CFG=None):
    ...
    df = pd.read_csv('data/NewsCategory/News_Category_Dataset_v3.json_bert.csv',
                     dtype=float, header=None)
    # df = df[df.iloc[:, -1].isin([0, 1, 2])] # only use the first three classes
...
def main():
    ...
    for train_val_seed in range(0, 1000, 1000 // NUM_REPEATS):
        ...
        data_dir = (f'data/NewsCategory/random_noise_model/h_{CFG.NUM_HONEST_CLIENTS}-b_{CFG.NUM_BYZANTINE_CLIENTS}'
    ...
    # history_file = 'data/NewsCategory/sign_flipping/h_12-b_8-5-0.8-0.1-krum_avg/all_histories_3.pth'
