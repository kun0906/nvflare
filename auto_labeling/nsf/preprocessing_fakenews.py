"""
    ...
    $PYTHONPATH=.:nsf python3 nsf/preprocessing_fakenews.py
...
@timer
def preprocessing():
    fake_file = 'data/FakeNews/Fake.csv'
    true_file = 'data/FakeNews/True.csv'
    # nrows=100
    nrows=None
    # Load datasets
    df_fake = pd.read_csv(fake_file, delimiter=',', nrows=nrows, encoding='ISO-8859-1', header=0)
    df_true = pd.read_csv(true_file, delimiter=',', nrows=nrows, encoding='ISO-8859-1', header=0)
    ...
    labels, texts = df['label'].tolist(), df['text'].tolist()
    ...
    csv_file = f'{fake_file}_bert.csv'
    buffer_size = 5000
