...
def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument("--workspace", "-m", default='/Users/49751124/PycharmProjects/nvflare/cifar10_demo/poc/example_project/prod_00/site-2/', type=str, help="WORKSPACE folder", required=False)
    parser.add_argument("--fed_client", "-s", default='startup/fed_client.json', type=str, help="client config json file", required=False)
    parser.add_argument("--set", default=['secure_train=false', 'uid=site-2', 'org=nvidia', 'config_folder=config'], metavar="KEY=VALUE", nargs="*")
    parser.add_argument("--local_rank", type=int, default=0)
    args = parser.parse_args()
    return args
