...
def parse_arguments():
    # -m .. -s fed_server.json --set secure_train=true  org=nvidia config_folder=config
    parser = argparse.ArgumentParser()
    parser.add_argument("--workspace", "-m", default='/Users/49751124/PycharmProjects/nvflare/cifar10_demo/poc/example_project/prod_00/server', type=str, help="WORKSPACE folder", required=False)
    parser.add_argument(
        "--fed_server", "-s", type=str, default='startup/fed_server.json', help="an aggregation server specification json file", required=False
    )
