...
class PTLearner(Learner):
    def __init__(
            self,
            train_path='~/data/reverse_label/client_2_train.pkl',
            test_path='~/data/reverse_label/client_2_test.pkl',
            site_id=2,
            lr=0.01,
            epochs=5,
            exclude_vars=None,
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        self.n_classes = np.unique(client_targets).size
        ...
        self.train_loader = DataLoader(self._train_subset, batch_size=32, shuffle=True)
        ...
        self.test_loader = DataLoader(self._test_subset, batch_size=32, shuffle=True)
    ...
    def local_train(self, fl_ctx, abort_signal):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                # if np.unique(labels).size != self.n_classes: continue
                # print(epoch, np.unique(labels).size, self.n_classes)
            ...
            self.writer.add_scalar("train_loss", float(running_loss), current_step)
    ...
    def local_validate(self, test_loader, data_type='test', abort_signal=None):
        ...
        pred_probs = torch.empty((0, self.n_classes), dtype=torch.float32)  # Adjust for all class probabilities
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(test_loader):
                ...
                # Predicted class is the one with the maximum probability
                _, pred_label = torch.max(output, 1)

                # Move tensors to CPU before concatenation
                pred_probs = torch.cat((pred_probs, pred_prob.cpu()))  # Storing probabilities for all classes
        ...
        pred_probs_np = pred_probs.numpy()  # Probabilities for all classes
        ...
        # Compute AUC
        if self.n_classes == 2:
            # Binary classification: Take the probabilities for the positive class
            pred_probs_np_binary = pred_probs_np[:, 1]  # Only the positive class probabilities
            auc = roc_auc_score(true_labels_np, pred_probs_np_binary)
        else:
            # Multi-class classification: Use One-vs-Rest (OvR) scheme
            from sklearn.preprocessing import label_binarize
            true_labels_binarized = label_binarize(true_labels_np, classes=list(range(self.n_classes)))
            auc = roc_auc_score(true_labels_binarized, pred_probs_np, multi_class='ovr')
        ...
        # # Compute AUC
        # auc = roc_auc_score(true_labels, pred_probs_np, multi_class='ovr')
        #
        # metrics[f'{data_type}_auc'] = auc
        # metrics[f'{data_type}_cm'] = cm_text
