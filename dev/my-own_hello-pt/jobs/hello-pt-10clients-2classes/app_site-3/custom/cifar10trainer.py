...
class Cifar10Trainer(Executor):
    def __init__(
        ...
        s = len(self._train_dataset) // 10
        subset_indices = torch.arange(s, 3 * s)  # For example, using only 10% of the dataset
        # Create a subset of the original dataset
        self._train_subset = Subset(self._train_dataset, subset_indices)    #

        # Get the size of the subset
        subset_size = len(self._train_subset)
        print(f"Client 1: Size of the training subset: {subset_size}")
        # Count the number of samples in each class within the subset
        class_counts = Counter()
        for _, label in self._train_subset:
